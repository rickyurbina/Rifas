    ...
    class BoletosController {
        ...
        public function ctrListarTodosLosNumeros($idSorteo){
          ...
          for ($i = 0; $i < 2500; $i++) {
            include_once "../../numeracion.php"; 
              $numeros = new calcula; 
              $boletos = $numeros -> oportunidadesRetorno($i);
            // if (($i+1) >= 1 && ($i+1) <668){
            //   $boletos = ''.($i+1).', '.(($i+1)+2000).', '.(($i+1) +4667).', '.(($i+1)+6667).', '.(($i+1)+8000).'';
            // }else if(($i+1) >= 668 && ($i+1) <1335){
            //   $boletos = ''.($i+1).', '.(($i+1)+2000).', '.(($i+1) +3333).', '.(($i+1)+5333).', '.(($i+1)+8000).'';
            // } else if(($i+1) >=1335 ){
            //   $boletos = ''.($i+1).', '.(($i+1)+2000).', '.(($i+1) +4000).', '.(($i+1)+6000).', '.((($i+1) != 2000) ? (($i+1)+8000) : ("0000")).'';
            // }
        ...
        public function ctrListarBoletosPendientes($idSorteo){
            ...
            foreach ($respuesta as &$item){
                ...
                //   $boletos = ''.$item["noBoleto"].', '.($item["noBoleto"]+2000).', '.($item["noBoleto"] +3333).', '.($item["noBoleto"]+5333).', '.($item["noBoleto"]+8000).'';
                // } else if($item["noBoleto"] >=1335 ) {
                //   $boletos = ''.$item["noBoleto"].', '.($item["noBoleto"]+2000).', '.($item["noBoleto"] +4000).', '.($item["noBoleto"]+6000).', '. (($item["noBoleto"] === 2000) ? '0000' : ($item["noBoleto"] + 8000)) . '';
                // }
