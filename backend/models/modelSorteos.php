...
class SorteosModel {
  ...
  public static function mdlActualizarSorteo($datosModel) {
    ...
    if ($datosModel['fotos'] !== '') {
      ...
      $statement = Conexion::conectar() -> prepare("UPDATE `sorteos` SET `titulo` = :titulo, `subtitulo` = :subtitulo, `fecha` = :fecha, `noBoletos` = :noBoletos, `costoBoleto` = :costoBoleto, `descripcion` = :descripcion, `segundoLugar` = :segundoLugar,  `tercerLugar` = :tercerLugar, `adicionales` = :adicionales, `fotos` = :fotos  WHERE `idSorteo` = :idSorteo;");
  ...
  public static function mdlListarSorteos() {
    ...
    $statement = Conexion::conectar() -> prepare("SELECT sorteos.idSorteo, sorteos.titulo,(15000-(COUNT(boletos.idBoleto))) as faltan, sorteos.fecha, sorteos.costoBoleto FROM sorteos INNER JOIN boletos ON sorteos.idSorteo=boletos.idSorteo GROUP BY sorteos.idSorteo;");
    $statement -> execute();
